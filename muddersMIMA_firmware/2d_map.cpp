...
uint16_t lastUsedMapIndex = 0;

uint16_t two_d_map_getLastUsedMapIndex() {
...
/* Example map:
   ...
   0.1 x 0.4 -> ([0, 0] * 0.9 * 0.6 + [0, 1] * 0.1 * 0.6 + [1, 0] * 0.9 * 0.4 + [1, 1] * 0.1 * 0.4)
			 ...
			 -> 2.64
 */
uint16_t evaluate_2d_map(uint16_t* map, uint16_t* dim1_map, uint8_t dim1_size, uint16_t dim1, uint16_t* dim2_map, uint8_t dim2_size, uint16_t dim2) {
	uint8_t dim1_res = 0;
	uint8_t dim2_res = 0;
	uint16_t dim1_rem = 0;
	uint16_t dim2_rem = 0;
	...
	uint16_t index = dim1_size * dim2_res + dim1_res;
	lastUsedMapIndex = index;

	uint16_t image = ((map[index] * (256 - dim1_rem)) / 256 * (256 - dim2_rem)) / 256;

	index += 1;
	image += ((map[index] * (dim1_rem)      ) / 256 * (256 - dim2_rem)) / 256;

	index += dim1_size - 1;
	image += ((map[index] * (256 - dim1_rem)) / 256 * (      dim2_rem)) / 256;

	index += 1;
	image += ((map[index] * (dim1_rem)      ) / 256 * (      dim2_rem)) / 256;
